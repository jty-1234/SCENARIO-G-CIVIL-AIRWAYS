...
The initial sizing procedure starts from an assumed maximum take-off mass $m_0$ and uses two primary design choices:
\begin{itemize}
\item wing loading $W/S$,
\item disk loading $W/A$,
\end{itemize}
together with mission requirements (stall speed $V_\mathrm{stall}$ and cruise speed $V_\mathrm{cruise}$). The goal is to obtain first-cut values of wing area $S$, total rotor disk area $A$, rotor diameter $D$, and the electrical power required in hover and cruise.

\subsubsection{Step 1: Convert mass to weight}
\begin{equation}
W = m_0 g ,
\end{equation}
where $g$ is gravitational acceleration.

\subsubsection{Step 2: Wing area from stall constraint and wing-loading choice}
Two lower bounds for the wing reference area $S$ are evaluated.
...
\paragraph{(c) Governing wing area}
The sizing wing area is taken as
\begin{equation}
S=\max\left(S_\mathrm{stall},\, S_{W/S}\right).
\end{equation}

\subsubsection{Step 3: Rotor disk area and rotor diameter from disk loading}
...
\subsubsection{Step 4: Hover electrical power from momentum theory}
...
\subsubsection{Step 5: Cruise electrical power from a parabolic drag polar}
